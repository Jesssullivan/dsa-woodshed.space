...
// client. Link resolution and TOC heading extraction also happen here, so
// neither the resolver nor $lib/docs/markdown crosses the boundary.
export const load: PageServerLoad = async ({ params }) => {
	...
	const html = await renderMarkdown(raw, { resolveLink: makeLinkResolver(sheet.sourcePath) });
	return {
		slug: sheet.slug,
		title: sheet.title,
